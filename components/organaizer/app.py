...
from src.service import MinioService, PresignedUrlRequest, PresignedUrlResponse
...
def get_presigned_put_urls() -> PresignedUrlResponse:
    return minio_service.generate_presigned_put_url(
        PresignedUrlRequest(**request.get_json())
    )
...
def get_execution(id: str) -> Optional[Execution]:
    ...
    if execution is not None and execution.status == "DONE":
        execution.source_image_url = minio_service.generate_presigned_get_url(
            PresignedUrlRequest(key=execution.key)
        ).url
        execution.predicted_image_url = minio_service.generate_presigned_get_url(
            PresignedUrlRequest(key=f"{execution.id}/result.jpg")
        ).url
    return execution
